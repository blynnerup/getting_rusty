...
// Immutable reference
fn inspect(s: &String) {
    if s.ends_with("s") { println!("plural") }
...
// Takes ownership of a string and returns value
fn eat(eating: String) -> bool{
    if eating.starts_with("b") && eating.contains("a") { return true }
    else { false } 
}
