...
fn main() {
    ...
    for arrow_coord in arrow_coords {
        ...
        if arrow_coord.distance_from_center() < 1.0 { shots.push(Shot::Bullseye) }
        else if arrow_coord.distance_from_center() > 1.0 && arrow_coord.distance_from_center() < 5.0 { shots.push(Shot::Hit(arrow_coord.distance_from_center())) }
    }
